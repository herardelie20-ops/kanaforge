...
function initCommunicationAI() {
  ...
  const legacyAppUrl = new URL('./', window.location.href).href;
  const storedWebsite = localStorage.getItem(websiteKey);
  const savedWebsite = storedWebsite === legacyAppUrl ? '' : (storedWebsite || '');
  agent.insertAdjacentHTML('beforeend', `<section class="website-connection"><small>SITE WEB PUBLIC · SÉPARÉ DU STUDIO</small><h3>Votre vitrine en ligne</h3><p>Ajoutez l’adresse de votre site public. Le Studio tatouage reste une application distincte, réservée à votre espace de travail.</p><label>Adresse du site<input id="website-url" type="url" inputmode="url" autocomplete="url" placeholder="https://votre-site.fr"/></label><div class="website-actions"><button id="website-open" type="button">Ouvrir le site</button><button id="website-copy" type="button">Copier le lien</button></div><button id="website-save" class="website-save" type="button">Enregistrer ce site</button><small id="website-status">Ajoutez l’URL de votre site public pour l’utiliser ici.</small></section>`);

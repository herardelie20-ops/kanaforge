  ...
  const paint = now => {
    ...
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        ...
        const cursorLine = ((x - focusX) * 0.62 + (y - focusY) * 0.78) / diagonalSpan;
        const cursorWave = Math.exp(-(cursorLine ** 2) * 42);
        ...
        const cursorSweep = cursorWave * (12 + h * 54);
        const relief = Math.max(0, 1 / normalLength) * h * 31;
        const sparkle = (primaryWave * .72 + cursorWave * .45) * (20 + Math.pow(h, 4.5) * 340);

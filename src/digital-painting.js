...
function initDigitalPainting() {
  ...
  if (!workspace) return;
  workspace.innerHTML = `<section class="digital-painting"><header><div><small>LIQUID IMAGE LAB</small><h2>Peinture, matière & liquéfaction</h2></div><div class="paint-header-actions"><button id="paint-fullscreen" type="button">⛶ Plein écran</button><label class="paint-import">Importer une image<input id="paint-file" type="file" accept="image/png,image/jpeg,image/webp"/></label></div></header><div class="paint-toolbar" role="toolbar" aria-label="Outils de peinture"><button class="active" data-paint-tool="stretch">Étirer</button><button data-paint-tool="swirl">Tourbillon</button><button data-paint-tool="contrast">Contraste</button><button data-paint-tool="black">Pinceau noir</button><button data-paint-tool="smooth">Lisser</button><button id="paint-auto" type="button">Auto flow</button></div><div class="paint-canvas-wrap"><canvas id="paint-canvas" width="1000" height="640" aria-label="Toile de peinture numérique"></canvas><span class="paint-cursor" aria-hidden="true"></span></div><p id="paint-status">Nuages monochromes prêts · faites glisser le pinceau sur la toile.</p></section><aside class="paint-controls"><p>RÉGLAGES DU PINCEAU</p><label>Taille <output id="paint-size-out">120 px</output></label><input id="paint-size" type="range" min="24" max="320" value="120"/><label>Pression <output id="paint-pressure-out">52 %</output></label><input id="paint-pressure" type="range" min="10" max="100" value="52"/><label>Grain <output id="paint-grain-out">16 %</output></label><input id="paint-grain" type="range" min="0" max="45" value="16"/><hr/><p>PALETTE SÉRIGRAPHIQUE</p><div class="paint-palettes" id="paint-palettes"><button class="active" data-paint-palette="noir">Noir & blanc</button><button data-paint-palette="encre">Encre</button><button data-paint-palette="lagon">Lagon</button><button data-paint-palette="braise">Braise</button></div><hr/><div class="paint-actions"><button id="paint-undo" type="button">↶ Annuler</button><button id="paint-reset" type="button">Réinitialiser</button><button id="paint-export" class="primary" type="button">Exporter le PNG</button></div><small>Le travail et l’historique restent sur cet appareil pendant la session.</small></aside>`;

...
function installShadingTools() {
  ...
  anchor.insertAdjacentHTML('beforebegin', `<section class="flash-shading-tools"><p>04 / REMPLISSAGE & OMBRAGE</p><small>Sélectionnez d’abord une forme vectorisée, puis appliquez une prévisualisation de remplissage.</small><div id="flash-fill-tools"><button data-fill="solid">Noir plein</button><button data-fill="dotwork">Dotwork</button><button data-fill="whip">Whip shading</button><button data-fill="soft">Dégradé doux</button><button data-fill="clear">Contour seul</button></div><label>Densité <output id="flash-fill-density-out">70 %</output></label><input id="flash-fill-density" type="range" min="15" max="100" value="70"/></section>`);
  ...
  const apply = fill => {
    ...
    if (!svg || !paths.length) { document.querySelector('#flash-line-status').textContent = 'Sélectionnez d’abord un contour fermé dans l’aperçu pour lui appliquer un remplissage.'; return; }
